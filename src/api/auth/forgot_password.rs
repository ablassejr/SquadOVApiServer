...
async fn forgot_pw(fa: &fusionauth::FusionAuthClient, login_id: &str) -> Result<(), super::AuthError> {
    match fa.start_forgot_password_workflow(login_id).await {
        ...
        Err(err) => match err {
            ...
            _ => Err(super::AuthError::System{
                message: format!("Start Forgot Password Workflow: {}", err),
            }),
        }
    }
}

async fn change_pw(fa: &fusionauth::FusionAuthClient, change_id: &str, password: &str) -> Result<(), super::AuthError> {
    match fa.change_user_password(change_id, password).await {
        Ok(_) => Ok(()),
        Err(err) =>Err(super::AuthError::System{
                message: format!("Change Password: {}", err),
        }),
    }
}
...
pub async fn forgot_pw_handler(data : web::Query<ForgotPasswordInputs>, app : web::Data<api::ApiApplication>) -> Result<HttpResponse, super::AuthError> {
...
pub async fn forgot_pw_change_handler(app : web::Data<api::ApiApplication>, data : web::Json<ChangePasswordInputs>) -> Result<HttpResponse, super::AuthError> {
