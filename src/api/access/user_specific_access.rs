...
pub trait UserIdSetObtainer {
    fn obtain(&self, req: &HttpRequest) -> Result<HashSet<i64>, common::SquadOvError>;
}
...
impl UserIdSetObtainer for UserIdPathSetObtainer {
    fn obtain(&self, req: &HttpRequest) -> Result<HashSet<i64>, common::SquadOvError> {
        ...
        let user_id = match user_id.parse::<i64>() {
            Ok(x) => x,
            Err(_) => return Err(common::SquadOvError::BadRequest),
        };
...
impl<T: UserIdSetObtainer> super::AccessChecker for UserSpecificAccessChecker<T> {
    fn check(&self, session: &SquadOVSession, req: &HttpRequest) -> Result<bool, common::SquadOvError> {
