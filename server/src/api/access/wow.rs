...
pub struct WowViewUserViewBasicData {
    pub view_uuid: Uuid,
    pub user_id: i64
}

pub struct WowViewUserPathObtainer {
    pub view_uuid_key: &'static str,
    pub user_id_key: &'static str
}

pub struct WowViewUserViewupChecker {
    pub obtainer: WowViewUserPathObtainer
}

#[async_trait]
impl super::AccessChecker<WowViewUserViewBasicData> for WowViewUserViewupChecker {
    fn generate_aux_metadata(&self, req: &HttpRequest) -> Result<WowViewUserViewBasicData, SquadOvError> {
        Ok(WowViewUserViewBasicData{
    ...
    async fn check(&self, app: Arc<ApiApplication>, session: &SquadOVSession, data: WowViewUserViewBasicData) -> Result<bool, SquadOvError> {
        Ok(session.user.id == data.user_id && app.check_wow_match_view_user_association(&data.view_uuid, data.user_id).await?)
    }

    async fn post_check(&self, _app: Arc<ApiApplication>, _session: &SquadOVSession, _data: WowViewUserViewBasicData) -> Result<bool, SquadOvError> {
        Ok(true)
    }
}