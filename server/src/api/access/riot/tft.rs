...
impl AccessChecker<TftMatchUuidData> for TftMatchAccessChecker {
    ...
    async fn check(&self, app: Arc<ApiApplication>, session: Option<&SquadOVSession>, data: TftMatchUuidData) -> Result<bool, SquadOvError> {
        let session = session.unwrap();
        // The user must be either in the match or a squad member of a user in that match.        
        let base_access_set: HashSet<i64> = HashSet::from_iter(app.list_squadov_accounts_in_tft_match(&data.match_uuid).await?);
    ...
    async fn post_check(&self, _app: Arc<ApiApplication>, _session: Option<&SquadOVSession>, _data: TftMatchUuidData) -> Result<bool, SquadOvError> {
        Ok(true)
    }
}