...
async fn register(fa: &fusionauth::FusionAuthClient, data: RegisterData) -> Result<(), squadov_common::SquadOvError> {
    ...
    match res {
        Ok(_) => Ok(()),
        Err(err) => Err(squadov_common::SquadOvError::InternalError(format!("Register {}", err))),
    }
}
...
pub async fn register_handler(data : web::Json<RegisterData>, app : web::Data<Arc<api::ApiApplication>>, req : HttpRequest) -> Result<HttpResponse, squadov_common::SquadOvError> {
