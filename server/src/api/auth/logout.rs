...
pub async fn logout_handler(app : web::Data<Arc<api::ApiApplication>>, req: HttpRequest) -> Result<HttpResponse, squadov_common::SquadOvError> {
    ...
    let session = match app.session.get_session_from_request(&req, &app.pool).await {
        ...
        Err(err) => return logged_error!(squadov_common::SquadOvError::InternalError(format!("Logout Find Session {}", err)))
    };
