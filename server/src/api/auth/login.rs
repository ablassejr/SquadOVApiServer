...
async fn login(fa: &fusionauth::FusionAuthClient, data: LoginData, ip: Option<&str>) -> Result<super::SquadOVSession, squadov_common::SquadOvError> {
    ...
    match res {
        ...
        Err(err) => match err {
            fusionauth::FusionAuthLoginError::Auth => Err(squadov_common::SquadOvError::Credentials),
            fusionauth::FusionAuthLoginError::Generic{code, message} => Err(squadov_common::SquadOvError::InternalError(format!("Code: {} Message: {}", code, message))),
            _ => Err(squadov_common::SquadOvError::InternalError(String::from("Unhandled error."))),
        }
    }
}
...
pub async fn login_handler(data : web::Json<LoginData>, app : web::Data<Arc<api::ApiApplication>>, req : HttpRequest) -> Result<HttpResponse, squadov_common::SquadOvError> {
    if app.is_logged_in(&req).await? {
        return logged_error!(squadov_common::SquadOvError::BadRequest);
    }
    ...
    match app.session.store_session(&session, &app.pool).await {
        ...
        Err(err) =>  logged_error!(squadov_common::SquadOvError::InternalError(format!("Store Session {}", err))),
    }
}