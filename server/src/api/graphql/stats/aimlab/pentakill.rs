...
impl api::ApiApplication {
    async fn get_aimlab_pentakill_data(&self, task: &str, mode: i32, user_id: i64, params: &super::GraphqlAimlabStatsParams) -> Result<Vec<stats::AimlabStatPentakillData>, SquadOvError> {
...
impl GraphqlAimlabPentakillStats {
    async fn ultimate(&self, context: &api::graphql::GraphqlContext, params: super::GraphqlAimlabStatsParams) -> FieldResult<Vec<stats::AimlabStatPentakillData>> {
        Ok(context.app.get_aimlab_pentakill_data(&self.task, 10, self.user_id, &params).await?)
    }
}