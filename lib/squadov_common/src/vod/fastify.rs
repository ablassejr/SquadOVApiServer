...
pub async fn fastify_mp4(input_fname: &std::path::Path, output_fname: &std::path::Path) -> Result<(), SquadOvError> {
    ...
    if !ffmpeg_output.status.success() {
        log::warn!("Failed to convert fastify mp4 with ffmpeg: {} to {}", input_fname.display(), output_fname.display());
