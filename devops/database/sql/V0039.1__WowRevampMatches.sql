...
CREATE TABLE new_wow_arenas (
    ...
    EXCLUDE USING GIST (combatants_key WITH =, instance_id WITH =, arena_type WITH =, tr WITH &&)
);
...
CREATE TABLE new_wow_encounters (
    ...
    instance_id INTEGER NOT NULL,
    EXCLUDE USING GIST (combatants_key WITH =, encounter_id WITH =, difficulty WITH =, instance_id WITH =, tr WITH &&)
);
...
CREATE TABLE new_wow_challenges (
    ...
    EXCLUDE USING GIST (combatants_key WITH =, instance_id WITH =, keystone_level WITH =, tr WITH &&)
);