...
BEGIN
    ...
    LOOP
        ...
        RETURNING character_id INTO combatant_character_id;
        ...
        SELECT wcl.user_id, new_event_id
...
CREATE OR REPLACE FUNCTION transfer_wow_match_combat_log_events(old_match_uuid UUID, in_match_view_uuid UUID, in_combat_log_uuid UUID, in_start TIMESTAMPTZ, in_end TIMESTAMPTZ)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    FOR tmp IN
        SELECT wcl.user_id, wa.*, wcl.uuid AS "combat_log_uuid"
        FROM wow_arenas AS wa
    ...
    LOOP
        ...
        PERFORM transfer_wow_match_combat_log_events(tmp.match_uuid, new_view_uuid, tmp.combat_log_uuid, tmp.tm, tmp.finish_time);
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FOR tmp IN
        SELECT wcl.user_id, wc.*, wcl.uuid AS "combat_log_uuid"
        FROM wow_challenges AS wc
        INNER JOIN wow_match_combat_log_association AS wcla
...
BEGIN
    ...
    FOR tmp IN
        SELECT wcl.user_id, we.*, wcl.uuid AS "combat_log_uuid"
        FROM wow_encounters AS we
        INNER JOIN wow_match_combat_log_association AS wcla
